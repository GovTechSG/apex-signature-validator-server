...
signatureValidatorController.$inject = [
  "$scope",
  "Notification",
  "testRequestService",
  "$sce",
...
function signatureValidatorController(
  $scope,
  Notification,
  testRequestService,
  $sce,
  ...
  function onHttpMethodChange() {
    ...
    if (controller.httpMethod === config.main.httpMethods[0]) {
      // GET
  ...
  function addSignature() {
    controller.signatures.push(
      new Signature(controller.apiUrl, {
  ...
  function compareBaseString(signature, generated, input) {
    ...
    for (let i = 0; i < generated.length; i++) {
      ...
      if (own == null) {
        ...
        bsResults +=
          "<span class='missing-basestring-char'>" + stringToAdd + "</span>";
        break;
      }
      if (gen !== own && !before) {
    ...
    if (input.length > generated.length) {
      ...
      bsResults +=
        "<span class = 'extra-basestring-char'>" +
        input.substring(generated.length) +
        "</span>";
    }
  ...
  function sendTestRequest() {
    ...
    if (controller.httpMethod !== "GET") {
      ...
      if (controller.requestBodyType === "application/x-www-form-urlencoded") {
        ...
        requestOptions.requestBody = controller.requestBody.urlencoded.reduce(
          (finalObject, currentObject) => {
            if (currentObject.key && currentObject.value) {
              // false if any of them are empty strings
    ...
    return testRequestService
      .sendTestRequest(controller.apiUrl, controller.httpMethod, requestOptions)
      .then(response => {
  ...
  function getCurrentConfig() {
    ...
    if (controller.httpMethod !== "GET") {
      ...
      currentConfig.requestBody.urlencoded = currentConfig.requestBody.urlencoded.map(
        urlencodedBody => {
          return {
  ...
  function getApiTestResponseClass() {
    return {
      ...
      "bg-warning":
        300 <= controller.apiTest.response.status &&
        controller.apiTest.response.status < 400,
      "bg-danger":
        (400 <= controller.apiTest.response.status &&
          controller.apiTest.response.status < 600) ||
        controller.apiTest.response.error
    };
  }
}
