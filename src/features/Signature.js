...
Signature.prototype.generateBaseString = function(httpRequestOptions) {
  if (!httpRequestOptions) {
    throw new Error(
      "Error generating base string: http request options required."
    );
  }
  if (
    isEmpty(this.signatureUrl) ||
    isEmpty(this.appId || isEmpty(this.timestamp) || isEmpty(this.nonce))
  ) {
    this.baseString = "";
    return;
  }
  ...
  if (
    httpRequestOptions.httpMethod !== "GET" &&
    httpRequestOptions.requestBodyType ===
      "application/x-www-form-urlencoded" &&
    httpRequestOptions.requestBody.urlencoded.length > 0
  ) {
    baseStringOptions.formData = httpRequestOptions.requestBody.urlencoded.reduce(
    ...
    if (apiUrl === "" || !apiUrl) {
      ...
      if (apexDomain !== -1) {
        ...
        let domainIdentifier =
          signature.gatewayZone === config.constants.gatewayZones.internet
            ? "e"
            : "i";
...
function authPrefix(signature) {
  return function() {
    if (signature.gatewayZone === config.constants.gatewayZones.internet) {
      ...
      signature.gatewayZone === config.constants.gatewayZones.intranet
    ) {
      switch (signature.authLevel) {
        case 1:
          return "apex_l1_ig";
...
function getBaseString(baseStringOptions) {
  ...
  let prefixedSignatureMethod =
    baseStringOptions.authPrefix.toLowerCase() + "_signature_method";
  let prefixedTimestamp =
    baseStringOptions.authPrefix.toLowerCase() + "_timestamp";
  ...
  if (baseStringOptions.queryString != null) {
    paramsArray = paramsArray.concat(
      parseParams(baseStringOptions.queryString)
    );
  }
  ...
  return (
    baseStringOptions.httpMethod.toUpperCase() +
    "&" +
    baseStringUrl +
    "&" +
    stringParams
